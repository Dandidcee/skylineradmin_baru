...
const AlertDialogTrigger = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof AlertDialogPrimitive.Trigger> & { render?: React.ReactNode }
>(({ className, render, children, ...props }, ref) => {
  if (render && React.isValidElement(render)) {
    return (
      <AlertDialogPrimitive.Trigger ref={ref} asChild className={className} {...props}>
        {React.cloneElement(render, { ...(children !== undefined ? { children } : {}) } as any)}
...
>(({ className, ...props }, ref) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-lg border bg-card p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 mx-4 w-[calc(100%-2rem)] max-w-lg",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Description
    ref={ref}
    className={cn("text-base text-text/60", className)}
    {...props}
  />
));
